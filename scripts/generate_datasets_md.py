...
def _render_commit_table(rows: list[tuple[type[BaseDataset], str]]) -> str:
    ...
    for cls, name in rows:
        m, c = cls.metadata, cls.parsed_counts or DatasetCounts()
        lines.append(
            f"| {m.publication_year} | {_fmt_name(cls, name)} "
            f"| {_fmt_int(c.vfcs)} | {_fmt_int(c.non_vfcs)} | {_fmt_int(c.projects)} "
            f"| {_fmt_paper(m)} |"
        )
    return "\n".join(lines) + "\n"
...
def _render_function_table(rows: list[tuple[type[BaseDataset], str]]) -> str:
    ...
    for cls, name in rows:
        ...
        lines.append(
            f"| {m.publication_year} | {_fmt_name(cls, name)} "
            f"| {_fmt_int(c.vulnerable_functions)} | {_fmt_int(c.benign_functions)} "
            f"| {_fmt_int(c.projects)} | {_fmt_paper(m)} |"
        )
    return "\n".join(lines) + "\n"
...
def _table_note(rows: list[tuple[type[BaseDataset], str]], total: int) -> str:
    """Footnote for the tables: the `*` gloss, plus a line when variants were collapsed."""
    note = _COUNTS_NOTE
    if len(rows) != total:
...
def render_readme(text: str) -> str:
    ...
    for marker, title, render in (
        ...
        block = _render_collapsed(title, len(rows), render(rows), _table_note(rows, len(classes)))
